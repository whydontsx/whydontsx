...
async def user_help_inline(client, inline_query):
    ...
    msg = f"<blockquote><b><emoji id=5215377245639549895>💎</emoji> ᴍᴇɴᴜ ɪɴʟɪɴᴇ <a href=tg://user?id={inline_query.from_user.id}>{inline_query.from_user.first_name} {inline_query.from_user.last_name or ''}</a>\n★ ᴛᴏᴛᴀʟ ᴍᴏᴅᴜʟᴇs: {len(HELP_COMMANDS)}\n  ᴘʀᴇꜰɪx: {' '.join(SH)}</b></blockquote>"
...
async def help_callback(client, callback_query):
    ...
    top_text = f"<blockquote><b><emoji id=5215377245639549895>💎</emoji> ᴍᴇɴᴜ ɪɴʟɪɴᴇ <a href=tg://user?id={callback_query.from_user.id}>{callback_query.from_user.first_name} {callback_query.from_user.last_name or ''}</a>\n★ ᴛᴏᴛᴀʟ ᴍᴏᴅᴜʟᴇs: {len(HELP_COMMANDS)}\n  ᴘʀᴇꜰɪx: {' '.join(SH)}</b></blockquote>"
    ...
    if mod_match:
        ...
        await callback_query.edit_message_text(
            text=text 
            + '\n<blockquote><b>ɢᴀʏᴏ-ᴜꜱᴇʀʙᴏᴛ ᴅᴇᴠ -<a href="tg://user?id=1344553362"> ɢᴀʏᴏᴡᴀɴɴ-ᴜʙᴏᴛ ᴠ1</a> </b></blockquote>',
            reply_markup=InlineKeyboardMarkup(button),
